...
import Modal from './modals'; // Import the Modal component for adding events
import EditEventModal from './editeventmodals'; // Import the EditEventModal component for editing/deleting events
import './Calendar.css';
...
function DemoApp() {
  ...
  const updateEventInDatabase = async (id, updatedEvent) => {
    const response = await fetch(`/api/schedulehours/${id}`, {
      method: 'PUT',
      headers: { 'Content-Type': 'application/json' },
